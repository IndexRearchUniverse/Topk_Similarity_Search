...
*    ./topk-earlyterm c [k] [the binary files] [maxDepth] [segpart]> [output file]
**/
#include "topk.h"
#include<fstream>
#include<cmath>
...
int hashNumEqualOne = 0, hashNumGreaterOne = 0, countNo = 0, hashCount  = 0;

int part , segments, partFilterNum = 0;
void topk::PerformTopk()
{
	int x_line, x_prefixLength, x_len, x_pos;	//记录x的各种参数，x由前缀事件堆顶取出
	...
	int prunningbound = recordNum / 2;
	int segment[2];
	double segmentsim, prunningsim = 0;
	bool flag = false, flags[2] = {false, false};
	segment[0] = recordNum / 3, segment[1] = recordNum * 2 / 3;
	...
	while(!prefixEventInstance.empty())
	{
		...
		for(int i = 0; i < invertedList[token].endingPostion; i++)
		{
			...
			sim_count = SimilarityUpperBoundAccess(sim_probe, joinRecord[y_line].bound);
			...
			else if(y_len >= x_len * (sk * sk)  && y_len <= x_len/(sk * sk))	//cosine
			{
				sizeCount++;
				if(((x_len - x_pos) < overlap) || ((y_len - y_pos) < overlap))
				{
