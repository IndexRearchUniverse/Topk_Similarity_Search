...
*    ./topk-multilen j [k] [the binary files] [maxDepth] [Batch]> [output file]
...
void topk::PerformTopk()
{
	...
	for(int j = 0; j < recordNum; j++)
	{
		...
		if(countNum % 5000000 == 0)
		{
			...
			cerr << setiosflags(ios::right) << setw(10) << multiLen << endl;
		}
...
/*
		multiLen = (int)((sim_probe + (1/(double)(x_len))-sk) * x_len) - 2;
		if(joinRecord[x_line].count == 1)
			multiLen = multiLen / 8;
		if(joinRecord[x_line].count == 2)
			multiLen = multiLen / 2;
*/
	/*
		if(sk < 0.720)
...
//	cout << inputk << " " << maxLen << " " << setiosflags(ios::fixed) << setprecision(3) << roundsim << " ";
//	cout << countNum << " " << totalNum << " ";
	endsk = sk;
