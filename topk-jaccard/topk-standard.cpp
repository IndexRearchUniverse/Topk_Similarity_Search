/**
* standard topk-similarity, the execute command is:
*    ./topk-standdard j [k] [the binary files] [maxDepth] > [output file]
**/
#include "topk.h"
#include<fstream>
...
unsigned int  hashNumEqualOne = 0, hashNumGreaterOne = 0;
...
void topk::PerformTopk()
{
	...
	while(!prefixEventInstance.empty())
	{
		...
		if(countNum % 1000000 == 0)
		{
			...
			cerr << setiosflags(ios::fixed) << setprecision(3) << sim_probe << endl;
		}
		...
		{
			int xlinepy, ylinepy, xpospy, ypospy;
			...
			if(joinRecord[x_line].rid == joinRecord[y_line].rid)
				continue;
			...
			if(sk < MINSK)
			{
				if(joinRecord[x_line].rid > joinRecord[y_line].rid)
				{
					xlinepy = y_line, xpospy = y_pos;
					ylinepy = x_line, ypospy = x_pos;
				}
				else
				{
					xlinepy = x_line, xpospy = x_pos;
					ylinepy = y_line, ypospy = y_pos;
				}
				if(joinRecord[xlinepy].moveRid == 0)
					joinRecord[xlinepy].moveRid = joinRecord[xlinepy].rid << 16;
				key = joinRecord[xlinepy].moveRid + joinRecord[ylinepy].rid;

				flag = HashChecked(key);
				if(flag == false)
				{
					continue;
				}
				...
				VerficationOptimization(xlinepy, ylinepy, xpospy, ypospy, key,  sk);
			}
			else if(y_len >= x_len * sk && y_len <= x_len/sk)
			{
				if(joinRecord[x_line].rid > joinRecord[y_line].rid)
				{
					xlinepy = y_line, xpospy = y_pos;
...
bool topk::HashChecked(unsigned long long key)
{
...
void topk::VerficationOptimization( int xline,  int yline, int xpos, int ypos, unsigned long long key, double& sk)
{
	verficationCount++;
	int xlen, ylen;
	int lx, ly;
...
void topk::CoutHashIndex()
{
	...
	cout << inputk << "\t" << maxDepth << "\tNULL\t";			//output k , maxdepth , segpart
